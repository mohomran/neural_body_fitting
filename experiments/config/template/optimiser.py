...
class Optimiser():
    ...
    def prepare_loss_ops(self, config):
        ...
        latent = self.predictions['latent']
        ...
        with tf.name_scope("generator_loss"):
            ...
            sqerr_latent = tf.zeros_like(latent)
            ...
            latent_subset = tf.gather(latent, tf.squeeze(tf.where(latent_flags)), axis=0)
            ...
            for k in loss_terms.keys():
                if k == 'model_parameters':
                    gen_loss_latent = tf.reduce_mean(loss_funcs[loss_terms[k][0]](latent_subset, smplparams_subset))
            ...
            sqerr_latent = sqerr(latent, smplparams)
            ...
            if 'latent_components' not in config.keys() or 'pose' in config['latent_components']:
                if np.all(config['latent_components'] == ['pose']):
                    ...
                    latent_pose = latent + self.latent_mean
                    smplparams_pose = smplparams + self.latent_mean
                else:
                    latent_pose = latent[:,10:226] + self.latent_mean[10:226]
                    smplparams_pose  = smplparams[:,10:226] + self.latent_mean[10:226]
                
                latent_quat = tf.py_func(conversions.rotmat_to_quaternion, [tf.reshape(latent_pose, [-1])], tf.float32)
                smplparams_quat = tf.py_func(conversions.rotmat_to_quaternion, [tf.reshape(smplparams_pose, [-1])], tf.float32)
                latent_quat = tf.reshape(latent_quat, [-1, 4])
                smplparams_quat = tf.reshape(smplparams_quat, [-1, 4])
                gen_quat_dist = tf.acos(2 * tf.square(tf.reduce_sum(tf.multiply(latent_quat, smplparams_quat), axis=1)) - 1)
            ...
            if False:
                gen_loss_jointsloc_posed = tf.reduce_mean(loss_funcs['msqerr'](jointsloc_proj_pred, 
                                           tf.py_func(compute_mpii_joint_locations, 
                                                      [latent_pose, jointsloc_betas_pred, jointsloc_posed_pred, latent[:,:10] + self.latent_mean[:10]], tf.float32)))
