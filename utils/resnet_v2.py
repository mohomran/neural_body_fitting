...
def bottleneck(inputs,
  ...
  with variable_scope.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
    ...
    residual = layers_lib.conv2d(
        preact, depth_bottleneck, [1, 1], stride=stride, scope='conv1')
    residual = resnet_utils.conv2d_same(
        residual, depth_bottleneck, 3, 1, rate=rate, scope='conv2')
    residual = layers_lib.conv2d(
        residual,
        depth, [1, 1],
        stride=1,
...
def bottleneck_38(inputs,
  ...
  assert len(widths) in [2, 3], "Only two variants are supported: 2-layer wide or 3-layer bottleneck."
  with variable_scope.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
    depth_in = utils.last_dimension(inputs.get_shape(), min_rank=4)
    preact = layers.batch_norm(
        inputs, activation_fn=nn_ops.relu, scope='preact')
    if widths[-1] == depth_in:
      shortcut = resnet_utils.subsample(inputs, stride, 'shortcut')
    else:
      shortcut = layers_lib.conv2d(
          preact,
    ...
    if len(widths) == 2:
        assert len(rate) == len(widths) #TODO: rename rate
        residual = resnet_utils.conv2d_same(
            preact, widths[0], 3, use_bias=False, stride=stride, rate=rate[0], scope='conv1')
        residual = layers_lib.conv2d(
            residual,
            widths[1], [3, 3],
            stride=1,
        ...
        residual = layers_lib.conv2d(
            preact, widths[0], [1, 1], stride=1, biases_initializer=None, scope='conv1')
        residual = resnet_utils.conv2d_same(
            residual, widths[1], 3, use_bias=False, stride=stride, rate=rate, scope='conv2')
        #TODO: add dropout depending on widths
        residual = layers_lib.conv2d(
            residual,
            widths[2], [1, 1],
            stride=1,
